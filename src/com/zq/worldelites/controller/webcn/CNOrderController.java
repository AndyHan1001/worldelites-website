...
public class CNOrderController{
	...
	//-------------------------------订单--------------------------------------------
		...
		public void order_vuserinfo(Model model,User user,String birthday_bg,HttpServletRequest request,HttpSession session){
			...
			List<Apartment> othersApartmentList=this.getOthersApartment(apartmentid, request);
		...
		public void orderjump(Model model,HttpServletRequest request,HttpSession session){
			...
			//大学。公寓
				if(apartment!=null){
					...
					Room room=this.getRoom(roomid);
					if(room!=null){
						model.addAttribute("oneroom", room);
					...
					order.setPrice("管理员计算中,请等待邮件通知");//总价
					...
					String htmlUrl="http://www.worldelites.com/order_cn/info_html.html?id="+user.getId()+"&apartmentid="+apartment.getId()+
							"&roomid="+room.getId();
