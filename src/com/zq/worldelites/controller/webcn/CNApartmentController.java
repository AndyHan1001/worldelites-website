...
@Controller() 
@RequestMapping("/apartment_cn")
public class CNApartmentController{
	...
	public void Apartment(Model model,Integer id,String name,HttpServletRequest request){
		...
		if(oneapartment.getFeature()!=null){
			String[] feature=oneapartment.getFeature_cn().split(",");
		...
		if(oneapartment.getAmenities()!=null){
			String[] amenities=oneapartment.getAmenities_cn().split(",");
		...
		if(oneapartment.getApartment_discountproject_cn()!=null){
			String[] utilities=oneapartment.getApartment_discountproject_cn().split(",");
		...
		if(allRoomList!=null&&allRoomList.size()>0){
			...
			for(Room oneRoom:allRoomList){
				if(oneRoom!=null&&oneRoom.getState()==1){
					...
					if(!StringUtil.isNullOrEmpty(oneRoom.getDiscountmethod_cn())){
						String[] discount=oneRoom.getDiscountmethod_cn().split(",");
		...
		if(oneapartment.getFeature_cn()!=null){
			String[] ss=oneapartment.getFeature_cn().split(",");
	...
	public String addcoupon(Integer id,HttpSession session,HttpServletRequest request){
		...
		if(session.getAttribute(Constants.FRONT_LOGIN)!=null){
			...
			if(!listaaa){
				...
				if(school==null){
					coupon.setSchoolid(0);
					coupon.setSchoolname("");
				}else{
