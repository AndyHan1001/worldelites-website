...
<body>
	...
	data-options="rownumbers:true,singleSelect:true,pagination:true,url:'getAtlasList.html',method:'get'">
		<thead>
		<th data-options="field:'ck',checkbox:true"></th>
		<th field="id" >流水号</th>
		<th field="ssid" >图集id</th>
		<th field="channel" >所属主体</th>
		<th field="parentid" >所属主体ID</th>
		<th field="type" >图集类型</th>
		...
		<th field="title" >图集标题</th>
		<th field="author" >图集作者</th>
		<th field="keyword" >关键词</th>
		<th field="note" >附记</th>
		<th field="source" >图集来源</th>
		<th field="memo" >摘要</th>
		<th field="content" >图集内容</th>
		<th field="logo" >图集缩略图</th>
		<th field="isenable" formatter="formatterisdel">图集是否通过审核</th>
		<th field="stick" formatter="formatterisdel">图集是否置顶</th>
		<th field="visit" >图集浏览次数</th>
		<th field="comment" >评论数量</th>
		<!-- <th field="isdel" formatter="formatterisdel">是否删除0否1是</th> -->
		</thead>
		<tbody>
    ...
    <!--添加窗口--> 
    <div id="atlasadd" class="easyui-window" title="添加新图集" data-options="modal:true,collapsible:false,maximizable:false,minimizable:false,closed:true,iconCls:'icon-save'" style="width:800px;height:600px;padding:10px;">
	    <form action="saveAtlas.html" method="post" name="atlasAS">
	    	<table cellpadding="5" border="0">
            	<caption style="font-size:20px;margin-bottom:10px;">添加图集
                  </caption>
	    		<tr>
	    			<td style="width:200px">流水号:</td>
	    			<td style="width:200px"><input onfocus="this.blur()"  readonly="readonly" name="id" ></input></td>
                    <td style="width:100px">图集ID:</td>
                ...
                <tr>
                    ...
                    <td>&nbsp;</td>
                    <td align="right">
                    	<input type="button" onClick="atlasA()" value="提交">&nbsp;&nbsp;
                        <script>
							function atlasA(){
								if($('[name="atlasAS"]').form('validate'))	
										$('[name="atlasAS"]').submit();
    ...
    <!--添加图集窗口--结束-->
    <!--修改窗口--> 
    <div id="atlasedit" class="easyui-window" title="修改新图集" data-options="modal:true,collapsible:false,maximizable:false,minimizable:false,closed:true,iconCls:'icon-save'" style="width:800px;height:600px;padding:10px;">
	    <form action="updateAtlas.html" method="post" name="atlasES">
	    	<table cellpadding="5" border="0">
            	<caption style="font-size:20px;margin-bottom:10px;">修改图集
                  </caption>
	    		<tr>
	    			<td style="width:200px">流水号:</td>
	    			<td style="width:200px"><input onfocus="this.blur()"  readonly="readonly" name="id" ></input></td>
                ...
                <tr>
                    ...
                    <td>&nbsp;</td>
                    <td align="right">
                    	<input type="button" onClick="atlasE()" value="提交">&nbsp;&nbsp;
                        <script>
							function atlasE(){
								if($('[name="atlasES"]').form('validate'))		
										$('[name="atlasES"]').submit();
			...
			pager.pagination({
				buttons:[{
					...
					handler:function(){
						...
						var row = $('#dg').datagrid('getSelected');
						$('#atlasedit').form('load', 'getAtlasById.html?id='+row.id);
						$.post("getAtlasById.html?id="+row.id,{},function(data){
							KindEditor.html('#content',data.content);							
							$('#atlasedit #ctime_es').datetimebox('setValue',data.ctime);
							$('#atlasedit #utime_es').datetimebox('setValue',data.utime);
							$('#atlasedit #btime_es').datetimebox('setValue',data.btime);
							$('#atlasedit #etime_es').datetimebox('setValue',data.etime);							
						},"json")
						$('#atlasedit').window('open');
					}
				},{
					iconCls:'icon-remove',
					handler:function(){
						...
						$.messager.confirm('确认','您确认想要删除记录吗？',function(r){    
						    if (r){ 
						    	var row = $('#dg').datagrid('getSelected');
						    	$.ajax({
								    url: 'delAtlasById.html?id='+row.id,
								    type: "GET",
								    dataType: "text",
