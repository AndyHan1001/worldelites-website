                    ...
                    <c:forEach items="${joinuslist}" var="item">
	                    <c:choose>
	                    	<c:when test="${item.id==classid}">
	                    		<li style="float:left;background-color:#FFF; background-image:url(<%=path2 %>/resources/web/images/arrow_y.png); background-repeat:no-repeat;"><a href="joinus.html?id=${item.id}">${item.title_cn }</a></li>
	                    	</c:when>
	                    	<c:when test="${item.id!=classid}">
	                    		<li><a href="joinus.html?id=${item.id}">${item.title_cn }</a></li>
	                    	</c:when>
	                    </c:choose>
