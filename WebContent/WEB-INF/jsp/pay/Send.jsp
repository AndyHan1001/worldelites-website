...
<%
//初始化定义参数
String v_mid,key,v_url,v_oid,v_amount,v_moneytype,v_md5info;  //定义必须传递的eile参数变量

	v_mid = Constants.PAY_ID;			   // 商户号，这里为测试商户号20000400，替换为自己的商户号即可
	v_url =  request.getAttribute("url").toString();    	 // 商户自定义返回接收支付结果的页面
													      		// MD5密钥要跟订单提交页相同，如Send.asp里的 key = "test" ,修改""号内 test 为您的密钥
	  key = Constants.PAY_KEY;			// 如果您还没有设置MD5密钥请登陆我们为您提供商户后台，地址：https://merchant3.chinabank.com.cn/
...
//****************************************
    ...
    v_oid=request.getAttribute("v_oid").toString();
	  if(v_oid!=null && !v_oid.equals(""))  //判断是否有传递订单号
	  {
	  }
		else                         
	  {
		...
		v_amount=request.getAttribute("v_amount").toString();				// 订单金额                   
		v_moneytype ="CNY";	                  				// 币种
...
String v_rcvname,v_rcvaddr,v_rcvtel,v_rcvpost,v_rcvemail,v_rcvmobile;  //定义非必需参数
	  ...
	  v_rcvname = "WE";     // 收货人
	  v_rcvaddr = "http://www.worldelites.com/";     // 收货地址
	   v_rcvtel = "+1 (510) 761-8000";      // 收货人电话
...
String v_ordername,v_orderaddr,v_ordertel,v_orderpost,v_orderemail,v_ordermobile;
   ...
   v_orderemail = request.getAttribute("email").toString();  // 订货人邮件
